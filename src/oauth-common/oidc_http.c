/*
 * Shared OIDC core for PgBouncer's OAuth validator modules.  See oidc_http.h.
 */
...
static size_t collect_body(char *ptr, size_t size, size_t nmemb, void *arg)
{
	...
	if (buf->overflow)
		return 0;
	if (buf->len + chunk > OIDC_HTTP_MAX_BODY) {
		buf->overflow = true;
...
bool oidc_http_init(char *errbuf, size_t errlen)
{
	...
	if (rc != CURLE_OK) {
		OIDC_SET_ERR(errbuf, errlen, "curl_global_init failed: %s", curl_easy_strerror(rc));
		return false;
	}
		       ...
		       struct oidc_http_response *resp, char *errbuf, size_t errlen)
{
	...
	curl = curl_easy_init();
	if (!curl) {
		OIDC_SET_ERR(errbuf, errlen, "cannot create a curl handle");
		return false;
	}
	...
	if (rc != CURLE_OK) {
		if (buf.overflow) {
			OIDC_SET_ERR(errbuf, errlen, "response from %s is larger than %d bytes",
				     url, OIDC_HTTP_MAX_BODY);
		} else {
			OIDC_SET_ERR(errbuf, errlen, "request to %s failed: %s", url,
